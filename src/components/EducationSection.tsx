          ...
          {EDUCATION.map((edu, i) => (
            <div key={i} className="p-8 glass-card rounded-[2rem] border-zinc-800/50 space-y-4 hover:bg-zinc-900/60 transition-all duration-500 group">
              <div className="flex justify-between items-start">
                ...
                <span className="text-[10px] font-mono font-bold text-zinc-400 uppercase tracking-widest">{edu.period}</span>
              </div>
              <div className="space-y-1">
                <p className="text-zinc-400 text-sm font-medium">{edu.degree}</p>
                <p className="text-[10px] font-mono uppercase tracking-widest text-zinc-400">{edu.location}</p>
              </div>
            </div>
          ))}
        </div>
